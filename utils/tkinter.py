...
def create_graph_with_tkinter():
    ...
    button_remove_node = Button(frame_buttons, text="Remove Node", command=lambda: tkinter_remove_node(graph, frame_nodes))
    button_remove_node.grid(row=0, column=1, padx=10, pady=10)

    button_add_edge = Button(frame_buttons, text="Add Edge", command=lambda: tkinter_add_edge(graph, frame_edges, text_node1, text_node2))
    button_add_edge.grid(row=1, column=0, padx=10, pady=10)

    button_remove_edge = Button(frame_buttons, text="Remove Edge")
    button_remove_edge.grid(row=1, column=1, padx=10, pady=10)

    text_node1 = Text(frame_buttons, height=1, width=8)
    text_node1.insert(1.0, "Node 1")
    text_node1.grid(row=2, column=0, padx=10, pady=10)

    text_node2 = Text(frame_buttons, height=1, width=8)
    text_node2.insert(1.0, "Node 2")
    text_node2.grid(row=2, column=1, padx=10, pady=10)
